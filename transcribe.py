...
class OnlineTranscriber:
    ...
    def inference(self, audio):
        ...
        if self.return_roll:
            ...
            on_pitches = np.squeeze(np.argwhere(out == 2)).tolist()
            reonset_pitches = np.squeeze(np.argwhere(out == 4)).tolist()
            if isinstance(onset_pitches, int):
                onset_pitches = [onset_pitches]
            if isinstance(off_pitches, int):
                off_pitches = [off_pitches]
            if isinstance(on_pitches, int):
                on_pitches = [on_pitches]
            if isinstance(reonset_pitches, int):
                reonset_pitches = [reonset_pitches]
            # print('after', onset_pitches, off_pitches)
            return onset_pitches, off_pitches, on_pitches, reonset_pitches
